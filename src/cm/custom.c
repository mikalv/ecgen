...
static custom_quadr_t custom_prime_input(const config_t *cfg, GEN order) {
	pari_sp ltop = avma;
...
static custom_quadr_t custom_prime_random(const config_t *cfg, GEN order) {
	pari_sp ltop = avma;
	custom_quadr_t result = {0};
	GEN p;
	GEN t = gen_0;
...
curve_t *custom_curve() {
	...
	if (cfg->random) {
		quadr = custom_prime_random(cfg, order);
	} else {
		quadr = custom_prime_input(cfg, order);
	}
